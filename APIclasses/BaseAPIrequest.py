...
class VKapi:
    ...
    def get_data(self) -> dict:
        ...
        params['attachments'] = self.__photo_path
        return params
    
    def ready_post(self) -> bool:
        """Готовность записи к постингу"""
        if self.__params['message'] is not None and self.__photo_path is not None:
            return True
        return False
    ...
    def __errors(self, response) -> list:
        ...
        if 'error' in response.text:
            return [True, self.error_codes[response.json()['error']['error_code']]]
